...
**E = DEFENSIVE MATRIX** (Tee: 'Brood War science-vessel matrix shield, lasts 10 sec'): `case 'matrix'` shields the ally hero nearest the cursor (else the most-hurt ally in 560, else self) for 320+110/lvl over 10s; fx `matrix` (hex-lattice bubble, follows, collapses when the shield pops) + `matrixhit` ripple from dealDamage. AI casts it on a hurt ally.
