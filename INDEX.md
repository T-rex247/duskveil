...
- Sprite v2: Grok plate `grok-imagine-image-quality` n=4 (prompt in scratchpad `gen_plates.py`; ⚠ `response_format:"url"` download 403s — use `b64_json`), plate 3 chosen → 3× `grok-imagine-video-1.5` 6s clips (`gen_videos.py`) → `tools/build_sheet.py --group`.
