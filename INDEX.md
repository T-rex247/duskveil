...
## DIRECTIONAL (360) SHEETS — 2026-08-21 — **8-WAY for GOLIATH** (Tee: 'Brood War Goliath 360 movement'): optional `_ne_`/`_se_` sheets (back-right / front-right quarters from the turntable @3.75s/0.75s → 6 clips), drawUnit picks 8 sectors (45°, 27° hysteresis in `u._sec`), SW/W/NW = mirrored se/e/ne (`u._mirror`). Recipe per legend: turntable → n,s,ne,se stills → 12 clips (~$6) → `--group key_{n,s,ne,se}`. — now GOLIATH, KORVAX, LIORA, CORWEN, RAVENER, BATTLECRUISER (stern/bow, 384 cells, `DIR_SCALE` evens out the per-group shared scale)
...
Tee: 'watch these two fight scenes, analyze all CGI elements, implement them; add a new character inspired by thanos and his cgi rings'. Built an ORIGINAL titan warlord `korvax` (fac `titan`, key `titan_korvax`, plate `korvax_2`, icons `korvax_0-3` + portrait from Grok). Kit = the duel's VFX: Q `pblast` (0.4s anchor → `lightning` jagged violet bolt, knockback, cracks/debris) · W `rings` (`mandala` fx orbiting 5s, lashes nearest foe every 0.7s, first hero hit gets `bound` = golden `bind` root 1.3s — stepUnit honours `u.bound`) · E `portal` (`ring` fx at origin + destination, 0.3s anchor, teleport + small AoE) · R `singularity` (`blackhole` fx pulls foes 1.6s host-side, then detonation column + teal shard spray + cracks; ult ceremony violet). Roster-wide CGI: every hero respawns through a golden `ring` portal; Battlecruiser ram = `comet` streak; Ravener brood = red `tendrils`; Brimstone gets 3 blue sky-lance `shell` telegraphs before impact. Faction color maps got `titan: '190,120,255'`. AI: korvax portals AWAY when low (shares the bastille branch). TikTok clips were pulled via Playwright network capture (yt-dlp fails on TikTok) — recipe in memory `reference_accessing_video_audio_content_matrix`.
...
**MK II 'battlecruiser meets Cybertruck' + true 360 (Tee eve):** `assets/anim/vectra_sovereign_top.png` = keyed TOP-DOWN plate (`cyberbc_top_2`, bow +x, atlas `bow_angle`); when a `${key}_top` sheet exists `drawRotatingUnit()` draws the sprite ROTATED by the smoothed heading (MOTION.turn lerp → heavy bank into turns), procedural 4-nacelle engine flare at the stern + turret flashes, hover lift; `drawUnitUI()` is the shared HP/shield painter. Matrix = SC-style soft sphere + absorb spark per hit (`f.hitA`), cracks only <40%. Portrait = `cyberbc_1` over the orbit matte.
**E = DRONE SPHERE** (Tee: 'drones swarm in an orb shield 360 around the ship forming a ball that slices through anything', 10s): `case 'orbdrones'` = self matrix shield + fx `orbsphere` (14 drones on 3 tilted counter-rotating rings r150, slicing tick 0.22s on anything touching the ball, outward shove, sparks). Previously **E = DEFENSIVE MATRIX** (Tee: 'Brood War science-vessel matrix shield, lasts 10 sec'): `case 'matrix'` shields the ally hero nearest the cursor (else the most-hurt ally in 560, else self) for 320+110/lvl over 10s; fx `matrix` (hex-lattice bubble, follows, collapses when the shield pops) + `matrixhit` ripple from dealDamage. AI casts it on a hurt ally.
**W = DRONE SWARM** (10 hull drones, `drone` fx, orbit/strafe, teal bolts, dock).
**R = ORBITAL JUDGMENT** (Tee: 'super final cinematic move — calls a satellite/bigger ship; master ship like the EDITH ring station, laser+blast like the Death-Star test fire'): `case 'judgment'` + fx `station` (keyed Grok plate `assets/plates/halo_station.png` = PLATES.halo, original ring station; flies in over the target, six green lances converge on the dish 1.15–2.3s, main beam 2.25–2.95s, climbs away 2.6–4.2s) + `whiteout` (screen flash) + mega fireball/columns/3 shock rings/fire zone/knockback at 2.3s (dmg 520+200/lvl, r 420). Sheets rebuilt at 384px cells; idle = keyed plate (sharp).
